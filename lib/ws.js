...
class BinanceusWS {
    ...
    constructor(beautify = true) {
        this._baseUrl = 'wss://stream.binance.us:9443/ws/';
        this._combinedBaseUrl = 'wss://stream.binance.us:9443/stream?streams=';
        this._sockets = {};
    ...
    onUserData(binanceusRest, eventHandler, interval = 60000) {
        return binanceusRest.startUserDataStream()
            .then((response) => {
                setInterval(() => {
                    binanceusRest.keepAliveUserDataStream(response);
                }, interval);
